...
for col in X.columns:
    # 增加指示特征，1表示缺失，0表示未缺失
    X_train_missing[col + '_missing'] = X_train_missing[col].isnull().astype(int)
    X_test_indicator[col + '_missing'] = X_test_indicator[col].isnull().astype(int) # 保持特征数一致

# 对原有的缺失值进行均值填充（使用训练集的均值）
train_means = X_train_missing[X.columns].mean()
X_train_indicator = X_train_missing.fillna(train_means)
X_test_indicator = X_test_indicator.fillna(train_means)

max_depths = [2, 5, 10, 15, None]
min_samples_leafs = [8, 6, 4, 2, 1]

print("=== 缺失值标记法 (新增指示列 + 均值填充) ===")
...
axes[0].set_title('训练集准确率热力图(新增指示列+均值填充)')
...
axes[1].set_title('测试集准确率热力图(新增指示列+均值填充)')
axes[1].set_xlabel('min_samples_leaf')
axes[1].set_ylabel('max_depth')

plt.tight_layout()
plt.savefig('task6_visualization.png', dpi=300)
